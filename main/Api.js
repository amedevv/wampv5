...
const sendImage = async (req, res) => {
    ...
    if (!fs.existsSync(`./main/whatsapp-session-${req.body.sender}.json`)) return res.status(400).json({ status: false, data: { msg: "Harap scan  QR code sebelum menggunakan API 2" } });
    const client = await loadSessionOrGenerateQR(req.body.sender);
...
module.exports = {
    sendMessage, sendImage, sendDocument, validateKey, sendButton, sendBlast, sendBlastMedia, phoneNumberFormatter
}
