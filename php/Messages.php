...
class Messages
{
    ...
    public function sendMsg($post)
    {
        ...
        if ($res['status'] === true) {
            setFlashMsg('success', 'Pesan berhasil dikirim');
            back('messages.php');
    ...
    public function storeSchedule($post)
    {
        ...
        if (mysqli_num_rows($cekScheduleNum) >= 100) {
            setFlashMsg('error', 'Anda sudah memiliki 100 pesan jadwal pending!');
            back('schedule.php');
        }
        ...
        setFlashMsg('success', 'Pesan Jadwal berhasil di tambahkan!');
        back('schedule.php');
    }
    ...
    public function deleteSchedule($status)
    {
        if (mysqli_query($this->db, "DELETE FROM schedule WHERE status = '$status' ") === true) {
            setFlashMsg('success', 'Berhasil hapus jadwal pesan ' . $status);
            back('schedule.php');
        }
    }
